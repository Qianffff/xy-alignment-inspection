...
def real_image(frame_width=1e-6,shift_x=None,shift_y=None,rotation=None,background_noise=True):
    
    center_pixel = int(np.round((pixels_real+1)/2))
    # Grid of secondary electron (SE) yields
    grid = np.zeros([pixels_real,pixels_real])
    
    # Cross dimensions in pixels
    cross_pixel_length_arm = int(np.round(cross_length/2/pixel_width_real))
    cross_pixel_halfwidth = int(np.round(cross_line_width/2/pixel_width_real))

    # Define some useful parameters about the cross geometry
    cross_pixel_left_side = int(np.round(center_pixel - cross_pixel_length_arm)-1)
    cross_pixel_right_side = int(np.round(center_pixel + cross_pixel_length_arm))
    cross_pixel_top_side = int(np.round(center_pixel - cross_pixel_length_arm)-1)
    cross_pixel_bottom_side = int(np.round(center_pixel + cross_pixel_length_arm))
    cross_pixel_half_width_plus = int(np.round(center_pixel + cross_pixel_halfwidth))
    cross_pixel_half_width_minus = int(np.round(center_pixel - cross_pixel_halfwidth)-1)

    # Random position and rotation cross
    max_shift = int(np.round(center_pixel - 10/8*cross_pixel_length_arm))
    ...
    if background_noise == True:
        grid += np.random.random([pixels_real,pixels_real])*background_noise_level
    
    # Define the position of the center of the cross relative to the corner of the image (in m)
    cross_center_x = int(np.round(pixels_real/2+shift_x))*pixel_width_real
    cross_center_y = int(np.round(pixels_real/2+shift_y))*pixel_width_real
