...
def simulate(procedure):
    ...
    while step <= steps:
        ...
        if step != 0:
            grid = grid[int(np.round(max(0,cross_center_measured_px[0]-frame_width/pixel_width_real/2))):
                        int(np.round(min(pixels_real,cross_center_measured_px[0]+frame_width/pixel_width_real/2))),
                        int(np.round(max(0,int(cross_center_measured_px[1]-frame_width/pixel_width_real/2)))):
                        int(np.round(min(pixels_real,int(cross_center_measured_px[1]+frame_width/pixel_width_real/2))))]
